...
- **BLE 双向流转**：一台作外设（GATT Server + 广播），另一台作中心（扫描 + 连接），角色可互换
...
| 页面 | 操作 |
|---|---|
| 连接 | 点「申请权限」→ 点「开启通知使用权」→ 一台点「作为外设」，另一台点「作为中心」 |
| 应用 | 可选：开启「仅转发选中」，勾选要转发的应用 |
| 设置 | 可选：改设备名、开「常驻后台」 |

连接成功后，任意一台收到通知，另一台会弹出 `<设备名> | <应用> | <标题>` 的通知。

> 建议先让一台开始广播（外设），再让另一台开始扫描（中心）。**无需在系统蓝牙里手动配对**。
...
├── ui/
│   ├── ConnectionFragment.kt   连接页（权限/角色/状态/日志）
